...
function watchPausable(source, cb, options = {}) {
  ...
  const stop2 = watchWithFilter(source, cb, __spreadProps(__spreadValues({}, watchOptions), {
    eventFilter
  }));
...
function useArrayFindLast(list, fn) {
  return computed2(() => toValue(!Array.prototype.findLast ? findLast(toValue(list), (element, index, array) => fn(toValue(element), index, array)) : toValue(list).findLast((element, index, array) => fn(toValue(element), index, array))));
}
...
function watchDeep(source, cb, options) {
  return watch2(source, cb, __spreadProps(__spreadValues({}, options), {
    deep: true
  }));
}
  ...
  const reuse = defineComponent({
    ...
    setup(props, { attrs, slots }) {
      return () => {
        ...
        const vnode = (_a = render.value) == null ? void 0 : _a.call(render, __spreadProps(__spreadValues({}, options.props == null ? keysToCamelKebabCase(attrs) : props), {
          $slots: slots
        }));
...
function useEventListener(...args) {
  ...
  const stopWatch = watchImmediate(() => {
    ...
    if (!(raw_targets == null ? void 0 : raw_targets.length) || !(raw_events == null ? void 0 : raw_events.length) || !(raw_listeners == null ? void 0 : raw_listeners.length))
      return;
    const optionsClone = isObject2(raw_options) ? __spreadValues({}, raw_options) : raw_options;
...
function onKeyDown(key, handler, options = {}) {
  return onKeyStroke(key, handler, __spreadProps(__spreadValues({}, options), { eventName: "keydown" }));
}
function onKeyPressed(key, handler, options = {}) {
  return onKeyStroke(key, handler, __spreadProps(__spreadValues({}, options), { eventName: "keypress" }));
}
function onKeyUp(key, handler, options = {}) {
  return onKeyStroke(key, handler, __spreadProps(__spreadValues({}, options), { eventName: "keyup" }));
}
...
function useBrowserLocation(options = {}) {
  ...
  const buildState = (trigger2) => {
    ...
    return reactive(__spreadValues({
      trigger: trigger2,
      state: state2,
      length,
      origin
...
function useCached(refValue, comparator = (a, b) => a === b, options) {
  const _a = options || {}, { deepRefs = true } = _a, watchOptions = __objRest(_a, ["deepRefs"]);
...
function useCloned(source, options = {}) {
  ...
  if (!manual && (isRef(source) || typeof source === "function")) {
    watch2(source, sync, __spreadProps(__spreadValues({}, options), {
      deep,
      immediate
    }));
  } else {
...
function useRefHistory(source, options = {}) {
  ...
  const manualHistory = useManualRefHistory(source, __spreadProps(__spreadValues({}, options), { clone: options.clone || deep, setSource }));
  ...
  return __spreadProps(__spreadValues({}, manualHistory), {
    isTracking,
    pause,
    resume,
    commit,
...
function useDebouncedRefHistory(source, options = {}) {
  ...
  const history = useRefHistory(source, __spreadProps(__spreadValues({}, options), { eventFilter: filter }));
  return __spreadValues({}, history);
}
...
function useDraggable(target, options = {}) {
  ...
  return __spreadProps(__spreadValues({}, toRefs2(position)), {
    position,
...
function combineCallbacks(combination, ...callbacks) {
  if (combination === "overwrite") {
    return async (ctx) => {
      ...
      if (callback)
        return __spreadValues(__spreadValues({}, ctx), await callback(ctx));
      return ctx;
    };
  } else {
    return async (ctx) => {
      for (const callback of callbacks) {
        if (callback)
          ctx = __spreadValues(__spreadValues({}, ctx), await callback(ctx));
      }
      return ctx;
    };
  }
  ...
  function useFactoryFetch(url, ...args) {
    ...
    if (args.length > 0) {
      if (isFetchOptions(args[0])) {
        options = __spreadProps(__spreadValues(__spreadValues({}, options), args[0]), {
          beforeFetch: combineCallbacks(_combination, _options.beforeFetch, args[0].beforeFetch),
          afterFetch: combineCallbacks(_combination, _options.afterFetch, args[0].afterFetch),
          onFetchError: combineCallbacks(_combination, _options.onFetchError, args[0].onFetchError)
        });
      } else {
        fetchOptions = __spreadProps(__spreadValues(__spreadValues({}, fetchOptions), args[0]), {
          headers: __spreadValues(__spreadValues({}, headersToObject(fetchOptions.headers) || {}), headersToObject(args[0].headers) || {})
        });
      }
    }
    if (args.length > 1 && isFetchOptions(args[1])) {
      options = __spreadProps(__spreadValues(__spreadValues({}, options), args[1]), {
...
function useFetch(url, ...args) {
  ...
  if (args.length > 0) {
    if (isFetchOptions(args[0]))
      options = __spreadValues(__spreadValues({}, options), args[0]);
    else
      fetchOptions = args[0];
  }
  if (args.length > 1) {
  ...
  const execute = async (throwOnFailed = false) => {
    ...
    const context = {
      url: toValue(url),
      options: __spreadValues(__spreadValues({}, defaultFetchOptions), fetchOptions),
      cancel: () => {
        isCanceled = true;
      }
    };
    ...
    if (timer)
      timer.start();
    return fetch(context.url, __spreadProps(__spreadValues(__spreadValues({}, defaultFetchOptions), context.options), {
      headers: __spreadValues(__spreadValues({}, headersToObject(defaultFetchOptions.headers)), headersToObject((_b = context.options) == null ? void 0 : _b.headers))
...
function useFileSystemAccess(options = {}) {
  ...
  async function open(_options = {}) {
    if (!isSupported.value)
      return;
    const [handle] = await window2.showOpenFilePicker(__spreadValues(__spreadValues({}, toValue(options)), _options));
  ...
  async function create(_options = {}) {
    if (!isSupported.value)
      return;
    fileHandle.value = await window2.showSaveFilePicker(__spreadValues(__spreadValues({}, options), _options));
    data.value = void 0;
    await updateData();
  }
...
function useImage(options, asyncStateOptions = {}) {
  const state = useAsyncState(() => loadImage(toValue(options)), void 0, __spreadValues({
    resetOnExecute: true
  }, asyncStateOptions));
...
function useInfiniteScroll(element, onLoadMore, options = {}) {
  ...
  const state = reactive(useScroll(element, __spreadProps(__spreadValues({}, options), {
...
function usePerformanceObserver(options, callback) {
  const _a = options, {
    window: window2 = defaultWindow,
    immediate = true
  } = _a, performanceOptions = __objRest(_a, [
    "window",
    "immediate"
  ]);
...
function usePointer(options = {}) {
  ...
  return __spreadProps(__spreadValues({}, toRefs2(state)), {
    isInside
  });
}
...
function useShare(shareOptions = {}, options = {}) {
  ...
  const share = async (overrideOptions = {}) => {
    if (isSupported.value) {
      const data = __spreadValues(__spreadValues({}, toValue(shareOptions)), toValue(overrideOptions));
      let granted = true;
    ...
    try {
      ...
      if (rawValue == null) {
        ...
        if (typeof mergeDefaults === "function")
          ...
          data.value = __spreadValues(__spreadValues({}, rawInit), value);
        else
          data.value = value;
      } else {
...
function useTitle(newTitle = null, options = {}) {
  ...
  if (options.observe && !options.titleTemplate && document2 && !isReadonly2) {
    useMutationObserver((_c = document2.head) == null ? void 0 : _c.querySelector("title"), () => {
...
function useWakeLock(options = {}) {
  ...
  if (isSupported.value) {
    ...
    whenever(() => documentVisibility.value === "visible" && (document2 == null ? void 0 : document2.visibilityState) === "visible" && requestedType.value, (type) => {
...
function useWindowScroll(options = {}) {
  const _a = options, { window: window2 = defaultWindow } = _a, rest = __objRest(_a, ["window"]);
